...
export async function GET(request: Request) {
  try {
    ...
    console.log(`🔄 Cron job running for ${products.length} products...`);
    ...
    const updatedProducts = await Promise.all(
      products.map(async (currentProduct) => {
        try {
          ...
          // Scrape product based on platform (Amazon, Snapdeal, etc)
          const scrapedProduct = await scrapeProductByPlatform(currentProduct.url);

          if (!scrapedProduct) {
            console.log(`⚠️ Failed to scrape: ${currentProduct.url}`);
            return null;
          }
          ...
          console.log(`✅ Updated: ${updatedProduct?.title} - New price: ₹${scrapedProduct.currentPrice}`);
          ...
          if (emailNotifType && updatedProduct.users.length > 0) {
            ...
            await sendEmail(emailContent, userEmails);
            console.log(`📧 Email sent for: ${updatedProduct.title}`);
          }
    ...
    console.log(`✅ Cron job completed: ${successCount}/${products.length} products updated`);
